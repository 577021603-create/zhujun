...
EMAIL_CONFIG = {
    ...
    "smtp_port": 465,  # SSL加密端口（已验证连通）
    "sender_email": "2332586642@qq.com",  # 发件人QQ邮箱
    "sender_password": "njszbcfndrzudjdc",  # 最新授权码（确保正确）
  #  "receiver_emails": ["577021603@qq.com", "bin.xu@cestc.cn","lizhonghua@cestc.cn","lijing07@cestc.cn"],  # 收件人列表
    "receiver_emails": ["577021603@qq.com","2374176823@qq.com", "1352388970@qq.com","zhu_jun@cestc.cn","panjun@cestc.cn","lizhonghua@cestc.cn","lijing07@cestc.cn","17621038018@163.com","17621038018@163.com",],
...
def send_vulnerability_email(email_subject: str, html_content: str) -> bool:
    ...
    try:
        ...
        # 构建邮件对象（适配QQ邮箱格式）
        msg = MIMEMultipart()
        msg['From'] = EMAIL_CONFIG["sender_email"]  # 直接用字符串，不包裹Header
        msg['To'] = ", ".join(EMAIL_CONFIG["receiver_emails"])  # 收件人拼接为字符串
        msg['Subject'] = Header(email_subject, 'utf-8')  # 主题保证中文正常

        # 添加HTML正文
        msg.attach(MIMEText(html_content, 'html', 'utf-8'))

        # 连接SMTP服务器（修正登录编码问题）
        with smtplib.SMTP_SSL(EMAIL_CONFIG["smtp_server"], EMAIL_CONFIG["smtp_port"], timeout=60) as server:
            # 直接用字符串登录，无需手动编码（smtplib内部自动处理）
            server.login(EMAIL_CONFIG["sender_email"], EMAIL_CONFIG["sender_password"])
            # 发送邮件
            server.sendmail(
                EMAIL_CONFIG["sender_email"],
                EMAIL_CONFIG["receiver_emails"],
                msg.as_string().encode('utf-8')  # 邮件内容编码为UTF-8
            )
        ...
        print(f'\n❌ 邮件发送失败：{error_msg}')
        print(f'❌ 排查建议：1. 确认授权码有效 2. 检查POP3/SMTP服务已开启 3. 查看邮箱发送记录')
        return False
...
if __name__ == "__main__":
    TARGET_URLS = [
        'https://www.secrss.com/articles/85951',
        # 可添加更多漏洞网页URL
    ]
